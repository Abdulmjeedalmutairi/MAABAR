...
const T = {
  ar: {
    ...
    actions: { submit_offer: 'قدّم عرضك ←', confirm_order: 'أكّد الطلب ←', approve_sample: 'وافق أو اعتذر ←', reply_inquiry: 'ردّ على الاستفسار ←', view_offer: 'عرض العرض ←', view_order: 'عرض الطلب ←', view_sample: 'عرض العيّنة ←', view_inquiry: 'عرض الاستفسار ←' },
    trader: 'تاجر', qty: 'الكمية', empty: 'لا شيء هنا الآن.', loading: 'جارٍ التحميل…',
  },
  en: {
    ...
    actions: { submit_offer: 'Submit your offer →', confirm_order: 'Confirm order →', approve_sample: 'Approve or decline →', reply_inquiry: 'Reply to inquiry →', view_offer: 'View offer →', view_order: 'View order →', view_sample: 'View sample →', view_inquiry: 'View inquiry →' },
    trader: 'Trader', qty: 'Qty', empty: 'Nothing here right now.', loading: 'Loading…',
  },
  zh: {
    ...
    actions: { submit_offer: '提交报价 →', confirm_order: '确认订单 →', approve_sample: '批准或拒绝 →', reply_inquiry: '回复咨询 →', view_offer: '查看报价 →', view_order: '查看订单 →', view_sample: '查看样品 →', view_inquiry: '查看咨询 →' },
    trader: '采购商', qty: '数量', empty: '暂无内容。', loading: '加载中…',
  },
};

export default function SupplierRequestsPanel({ sb, supplierId, lang = 'ar', onAction }) {
  const c = T[lang] || T.ar;
  const isAr = lang === 'ar';
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--text-disabled)', marginBottom: 18, ...arFont }}>{c.sub}</p>
