...
export default function Home({ lang, user, displayCurrency }) {
  const nav = useNavigate();
  const t = T[lang] || T.ar;
  const isAr = lang === 'ar';
  usePageTitle('home', lang);
  const [products, setProducts] = useState([]);
  const [ideaOpen, setIdeaOpen] = useState(false);   // "Request manufacturing" (idea-to-product) modal
    ...
    <div ref={rootRef} className={`home2${isAr ? ' rtl' : ''}`}>
      ...
      <section className="home2-cta reveal">
        <div className="home2-cta-body">
          ...
          <button className="btn-primary" onClick={() => nav('/request')}>{t.ctaBtn}</button>
        </div>
      ...
      {ideaOpen && (
        <IdeaToProduct lang={lang} user={user} displayCurrency={displayCurrency} onClose={() => setIdeaOpen(false)} />
      )}
    </div>
  );
}
