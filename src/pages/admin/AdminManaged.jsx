...
const TABS = [
  ...
  { key: 'assigned',  en: 'Assigned',  ar: 'معيّن' },
  { key: 'sourcing',  en: 'Sourcing',  ar: 'بحث' },
    ...
    <AdminRouteGuard user={user} profile={profile} lang={lang}>
      <AdminShell user={user} profile={profile} lang={lang}>
        ...
        <div className="a-page" dir={isAr ? 'rtl' : 'ltr'}>
          <h1 className="a-page-title">{isAr ? 'الطلبات المُدارة' : 'Managed Requests'}</h1>
          ...
          <input className="a-search" placeholder={isAr ? 'بحث بالعنوان أو التاجر...' : 'Search by title or buyer…'} value={search} onChange={e => setSearch(e.target.value)} dir={isAr ? 'rtl' : 'ltr'} />
          ...
          <div className="a-table-wrap">
            <table className="a-table">
              ...
              <tbody>
                ...
                {!loading && filtered.length === 0 && <tr><td colSpan={6}><div className="a-empty">{isAr ? 'لا توجد طلبات' : 'No requests found'}</div></td></tr>}
                {filtered.map(r => (
                  <tr key={r.id} onClick={() => nav(`/admin/managed/${r.id}`)}>
                    <td>
                      <div style={{ fontWeight: 500, color: 'rgba(0,0,0,0.85)' }}>{r.title || '—'}</div>
                      {r.target_price && <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)', marginTop: 1, fontFamily: FONT_BODY, fontVariantNumeric: 'lining-nums' }}>{r.target_price} {r.currency}</div>}
                    </td>
                    <td>
                      <div>{r.buyer?.full_name || '—'}</div>
                      {r.buyer?.company_name && <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)', marginTop: 1 }}>{r.buyer.company_name}</div>}
                    </td>
                    <td>{r.category || '—'}</td>
                    <td><AdminStatusBadge status={r.status} lang={lang} /></td>
                    <td>{r.trader_decision ? <AdminStatusBadge status={r.trader_decision} lang={lang} /> : <span style={{ color: 'rgba(0,0,0,0.25)' }}>—</span>}</td>
          ...
          <div className="a-cards">
            ...
            {filtered.map(r => (
              <div key={r.id} className="a-card" onClick={() => nav(`/admin/managed/${r.id}`)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, fontFamily: FONT_BODY, color: 'rgba(0,0,0,0.85)' }}>{r.title || '—'}</div>
                  <AdminStatusBadge status={r.status} lang={lang} />
                </div>
                <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
                  {[
                    [isAr ? 'التاجر' : 'BUYER', r.buyer?.full_name],
                    [isAr ? 'الفئة' : 'CATEGORY', r.category],
